...
beta=4;
q_0_sam=[0 0.25 0.5 0.75];                                                               % flipping probabilities
q_1=0;
...
Pdst = zeros(size(PFA));
w0=zeros(N,MC);
PD0G=zeros(length(PFA),length(q_0_sam));
PDSG=zeros(length(PFA),length(q_0_sam));
PD0R=zeros(length(PFA),length(q_0_sam));
PDSR=zeros(length(PFA),length(q_0_sam));
PD0T=zeros(length(PFA),length(q_0_sam));
PDST=zeros(length(PFA),length(q_0_sam));
step=200;
lambda_defult=1/2*pi/(1-0.7-q_1)^2;                     % default FI, Chapter 6, Kay                                                          
theta = sqrt(lambda_defult/(2*alpha^2*N)); 
h = 1;

for q_0_index=1:length(q_0_sam)
    q_0=q_0_sam(q_0_index);
    %% Generate GGN
for i=1:N/step
    for j=1:MC/step
        waitbar((100*(q_0_index-1)+10*(i-1)+j)/400);
...
for i = 1:length(PFA) 
    ...
    PD0G(1:length(PFA),q_0_index)= Pd0G;
    PDSG(1:length(PFA),q_0_index)= PdsG;
    PD0R(1:length(PFA),q_0_index)= Pd0R;
    PDSR(1:length(PFA),q_0_index)= PdsR;
    PD0T(1:length(PFA),q_0_index)=Pd0t;
    PDST(1:length(PFA),q_0_index)=Pdst;
end
%% figure
